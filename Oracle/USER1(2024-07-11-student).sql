...
CHAR : 고정문자열, 길이가 일정한 문자열 데이터를 저장하기
    CHAR type 에 순수 숫자형 문자열을 저장하면 SELECT에서 문제를 일으키는 경우가 많다
...
오라클 9 이하의 버전에서 문자열의 type VARCHAR2, nVARCHAR2 로 명확히 구분을 하는데
만약에 칼럼에 한글 "홍길동"이 저장되어 있다면
VARCHAR2 는 문자열의 길이가 6으로 nVARCHAR2 는 문자열의 길이가 3으로 표현된다.

현재 사용하는 21c 버전에서는 한글이나 영문의 문자열 길이를 실제 문자열의 개수로 표현한다.
하지만 9 이전버전은 VARCHAR2는 byte 크기로 표현하고, nVARCHAR2 는 문자열의 길이로 표현하여
혼란을 준다

현재버전에서 VARCHAR2 와 nVARCHAR 를 구분하는 것은 호환성 등을 위한 것이다.

DB 버전에 따라 VARCHAR2 는 실제 길이보다 한글 문자열을 절반밖에 저장 못 하는 경우도 있다.
...
*/
/*
INSERT INTO 를 실행할 때 전체칼럼에 일치하는 데이터를 추가 할 때 
INSERT 되는 칼럼 리스트를 생략할 수 있지만,
이는 데이터 무결성을 해칠 수 있는 아주 위험한 코드이다 절대 사용 금지

아래의 INSERT 코드는 절대 사용하지 말자
INSERT INTO tbl_student  VALUES ('2400001','홍길동','정보통신','3','010-1111-1111');
...
/*
학생테이블(tbl_student)에서 이몽룡 학생의 전화번호를 010-3333-3333 에서 010-3333-3456 으로 변경하는 SQL 을 작성

UPDATE, DELELE 명령문은 2개 이상의 레코드가 변경되지 않도록 실행해야한다.
...
-- st_num 칼럼은 PK 도 아니며, UNIQUE 칼럼도 아니다.
...
-- 학생데이터 중에 학과가 국사학과 전공인 학생의 리스트를 보여라
...
-- 학생테이블 전체 칼럼(속성,Attribute)중에서 특정한 칼럼만 지정하여 화면에 보이도록 하는 것을 Projection 이라고 한다
SELECT st_name, st_tel from tbl_student ORDER BY st_name;

gd_num	VARCHAR(5)		PRIMARY KEY
gd_kor	NUMBER		
gd_eng	NUMBER		
gd_math	NUMBER		
gd_history	NUMBER		
